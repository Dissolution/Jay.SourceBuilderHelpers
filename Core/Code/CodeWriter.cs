...
public partial class CodeWriter : IDisposable
{
    ...
    public CodeWriter AutoGeneratedHeader(string? comment = null)
    {
        ...
        return WriteLine("// <auto-generated>")
            .IndentBlock("// ", ib => { ib.WriteRangeLines(comment, lines); })
        ...
        switch (lines.Count)
        {
            ...
            default:
                return Write("/* ").WriteLine(comment![lines[0]])
                    .IndentBlock(" * ", ib => { ib.WriteRangeLines(comment, lines.Skip(1)); })
                    .WriteLine(" */");
        }
    }
    ...
    {
        return _length == 0 || char.IsWhiteSpace(_charArray[^1]);
    }
    ...
    private void WriteRangeLines(string main, IEnumerable<Range> ranges)
    {
        foreach (var range in ranges)
        {
            WriteLine(main[range]);
        }
    }
